...
class Checkout Extends CI_Controller {
  ...
  public function __construction(){
    ...
    public function process() {
        ...
        $config = array(
          'PayPalMode' => $this->config->item('PayPalMode'),      // Sandbox / testing mode option.
          'PayPalApiUsername' => $this->config->item('PayPalApiUsername'),  // PayPal API username of the API caller
          'PayPalApiPassword' => $this->config->item('PayPalApiPassword'),  // PayPal API password of the API caller
          'PayPalApiSignature' => $this->config->item('PayPalApiSignature'),  // PayPal API signature of the API caller
          'APISubject' => '',                   // PayPal API subject (email address of 3rd party user that has granted API permission for your app)
          'APIVersion' => $this->config->item('APIVersion'),   // API version you'd like to use for your call.  You can set a default version in the class and leave this blank if you want.
          'PayPalCurrencyCode' => $this->config->item('PayPalCurrencyCode'),
          'PayPalReturnURL' => $this->config->item('PayPalReturnURL'),
          'PayPalCancelURL' => $this->config->item('PayPalCancelURL')
        );

        $paypalmode = ($config['PayPalMode'] == 'sandbox') ? '.sandbox' : '';

        // echo '<pre>';
        // print_r($cart = $this->cart->contents());
        // exit();

        if ($cart = $this->cart->contents()) { //Post Data received from product list page.
            ...
            $ItemName = $cart["name"]; //Item Name
            $ItemPrice = $cart["price"]; //Item Price
            $ItemNumber = $cart["itemnumber"]; //Item Number
            $ItemDesc = $cart["itemdesc"]; //Item Number
            $ItemQty = $cart["qty"]; // Item Quantity
